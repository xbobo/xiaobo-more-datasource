...
//  Serif、SansSerif、Monospaced、Dialog 和 DialogInput 随意选择
	...
	public static float font_sacle = 1.1F;
	...
	public static BufferedImage zoomBySize(BufferedImage srcImage, int width, int height) throws IOException {
		...
		Image _img = srcImage.getScaledInstance(width, height, Image.SCALE_SMOOTH);
	...
	public static BufferedImage addPriceIcon(BufferedImage srcImage, String price) {
		...
		graphics.fillRoundRect(width - width / scale - len * size, height - size * 3 + size / 2, size * len - size,
				size * 2, width / radioScale, width / radioScale);
		// 释放对象 透明度设置结束
		graphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER));
		Font font = new Font(PictureUtilTest.FONT_STYLE, Font.BOLD, size);
	...
	public static String buildImageInsertLogo(String strurl, String content, String codeUrl, String price)
		...
		int org_width = srcImage.getWidth();
		int org_height = srcImage.getHeight();
//		if(org_width>3000) {
//			srcImage=zoomBySize(srcImage, 1125, 630);//715  400 535, 300
//		}else if(org_width<1000) {
//			srcImage=zoomBySize(srcImage, 1125, 630);// 358 200
//		}
		srcImage = zoomBySize(srcImage, 535, 300);
		...
		int size = (int)(_width / ROW_LEN*font_sacle);
		if (coententLen > 0) {
		...
		BufferedImage destImage = new BufferedImage(_width, show_height, BufferedImage.TYPE_INT_RGB);
		initBackageColor(destImage);
		int curHeight = padding;
		int contentlength = content.length();
		BufferedImage codeBufferimage = QrcodeUtil.imageInsertLogo(codeUrl,(int) (_width / 3*font_sacle));
		// codeBufferimage = zoomByScale(codeBufferimage, 0.35);
		...
		curHeight = curHeight + _height + size / 2;
		// 添加标题
	...
	private static int addContentToBufferImageContents(BufferedImage destImage, int curHeight, String content,
			int contentlength) {
		if (contentlength > 0) {
			int size =(int) (destImage.getWidth() / ROW_LEN *font_sacle);
...
//		graphics.drawRect(startX, y, srcImage.getWidth(), srcImage.getHeight());
	...
	public static void addContentToBufferImageSimple(BufferedImage destImage, int y, String strContent) {
		...
		Graphics2D graphics = (Graphics2D) destImage.getGraphics();
		int fontsize = destImage.getWidth() / ROW_LEN ;
		...
		Font font = new Font(FONT_STYLE, Font.TRUETYPE_FONT, fontsize);
		graphics.setColor(Color.BLACK);
		MyFont myFont = new MyFont();
		Font definedFont2 = myFont.getDefinedFont(Font.BOLD, fontsize*font_sacle);
		graphics.setFont(definedFont2);
		int dwidth = destImage.getWidth();
		int strWidth = getStringLength(graphics, strContent);
		float startX = 0F;
		float row_sacle=1.1F;
		if (dwidth > strWidth) {
			startX = (dwidth - strWidth*row_sacle) / 4;
		}
		// 直接绘制
		if(StringUtils.isNotEmpty(strContent)) {
			float tempX=Float.valueOf(startX+"");
			int len=strContent.length();
			for(int i=0;i<len;i++) {
				String tempStr=strContent.substring(i,i+1);
				if(i>0) {
					tempX=tempX+fontsize*font_sacle*row_sacle;
				}
				graphics.drawString(tempStr, tempX, y);
			}
		}
	...
	public static void addContentToBufferImageSimpleTips(BufferedImage destImage, int y, String strContent) {
		...
		Graphics2D graphics = (Graphics2D) destImage.createGraphics();
		Font font = new Font(FONT_STYLE, Font.ROMAN_BASELINE, size);
		graphics.setColor(Color.gray);
		TipsFont tipsFont = new TipsFont();
		Font definedFont = tipsFont.getDefinedFont(Font.PLAIN, size*font_sacle);
		graphics.setFont(definedFont);
		int dwidth = destImage.getWidth();
		int strWidth = getStringLength(graphics, strContent);
		int startX = 0;
