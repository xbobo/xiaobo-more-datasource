...
import org.xiaobo.mybatis.moredatasource.repository.secondary.MessageRepository;
...
public class MessageServiceImpl implements MessageService {
	...
	@Override
	public boolean  saveBatch(List<Message> list) {
		int saveBatch = messageRepository.saveBatch(list);
		for(Message m:list) {
	...
	@Override
	public boolean updateBatch(List<Message> list) {
		int updateBatch = messageRepository.updateBatch(list);
	...
	public boolean updateStatusByIds(Integer status, Long[] ids) {
		int updateBatchBySql = messageRepository.updateStatusByIds(status, ids);
		return updateBatchBySql>0;
	}

	@Override
	public boolean removeBatch(List ids) {
		int removeBatch = messageRepository.removeBatch(ids);
		return removeBatch>0;
	}
